...
def _infer_branch(repo: Optional[Path]) -> str:
    ...
    --advance and --push name a branch; when the caller omits --branch the
...
def cmd_blob_commit(base: str, branch: str, message: str, paths: List[str],
    ...
    needs a human.
    """
    if (advance or push) and not branch:
        branch = _infer_branch(repo)
        if not branch:
            print("vcs: --advance/--push need a branch and this checkout is on "
...
def cmd_read(ref: str, path: str, out: str = "", force: bool = False,
             repo: Optional[Path] = None) -> int:
    ...
    So this separates the three outcomes a raw read conflates:
      ...
      exit 3  --out would overwrite a file carrying UNCOMMITTED work (pass
              --force if discarding it is the point)
    """
    root_r = _git(repo, "rev-parse", "--show-toplevel")
    if root_r.returncode != 0:
        return _die("not inside a git repository")
    ...
    if br.returncode != 0:
        ...
        return 1
    if out:
        # REFUSE to overwrite a file carrying UNCOMMITTED work. --out is a
        # destructive write, and in a shared worktree the thing it destroys is
        # usually a peer's in-flight edit -- not yours, and not recorded
        # anywhere you will look. Measured 2026-09-05: refreshing
        # AitherOS/config/ecosystem.yaml from the branch tip silently reverted
        # 103 uncommitted lines including another session's entire brick
        # registration. It was recoverable ONLY because that session happened
        # to have staged it (`git show :<path>`); had it been unstaged it was
        # gone with no error, no diff and no reflog.
        #
        # blob-commit ALREADY refuses a shrinking copy at COMMIT time and says
        # so loudly -- and it points at this very command as the remedy, which
        # is how a guard against sweeping a peer became the way to sweep one.
        # The two must agree, so the refusal lives here too.
        if not force:
            st = _git(root, "status", "--porcelain", "--", out)
            dirty = st.stdout.strip() if st.returncode == 0 else ""
            if dirty:
                print(f"vcs: REFUSING to overwrite {out} -- it carries "
                      f"UNCOMMITTED changes ({dirty.splitlines()[0].strip()})")
                print("vcs: --out is a destructive write. In a shared worktree "
                      "the work you would destroy is usually a peer's.")
                print("vcs: read it somewhere else and diff first:")
                print(f"vcs:   awgit read {ref} {path} --out {out}.theirs")
                print(f"vcs:   diff {out}.theirs {out}")
                print("vcs: or pass --force if discarding those changes IS the point.")
                return 3
...
def selftest() -> int:
    ...
    try:
        ...
        rc = cmd_blob_commit("HEAD", "", "detached", ["mine.txt"], repo=td,
                             advance=True)
        assert rc == 2, f"detached HEAD with --advance must refuse (rc={rc})"
        assert (_git(td, "rev-parse", "main").stdout.strip() == before), (
            "detached refusal moved main")
        _git(td, "checkout", "-q", "main")
        rc = cmd_blob_commit("HEAD", "", "inferred", ["mine.txt"], repo=td,
                             advance=True)
        assert rc == 0, f"inferred-branch --advance failed (rc={rc})"
        assert (_git(td, "rev-parse", "main").stdout.strip() != before), (
            "inferred-branch --advance did not move main")
        assert (_git(td, "show", "main:mine.txt").stdout == "v2" + chr(10)), (
            "advanced commit lacks the named path content")
        ...
        # read --out: a DESTRUCTIVE write, and the thing it destroys in a
        # shared worktree is usually a peer's uncommitted edit. Measured
        # 2026-09-05 before this guard existed: refreshing a config from the
        # branch tip reverted 103 uncommitted lines of another session's work,
        # with no error and no reflog. Three arms, because the guard fails in
        # three different directions and only one of them is loud.
        _out_new = td / "read_out_new.txt"
        rc = cmd_read("HEAD", "ledger.md", out=str(_out_new), repo=td)
        assert rc == 0, "--out to a NEW path was refused — the guard cries wolf"
        assert _out_new.exists(), "--out reported success and wrote nothing"

        # the arm that matters: a target carrying uncommitted work is REFUSED,
        # and — the half a return-code assertion cannot see — that work SURVIVES.
        _dirty = td / "ledger.md"
        _keep = _dirty.read_text(encoding="utf-8")
        _dirty.write_text(_keep + "PEER EDIT NOT YET COMMITTED\n",
                          encoding="utf-8")
        rc = cmd_read("HEAD", "ledger.md", out=str(_dirty), repo=td)
        assert rc == 3, "--out overwrote a file carrying uncommitted work"
        assert "PEER EDIT NOT YET COMMITTED" in _dirty.read_text(
            encoding="utf-8"), (
            "the guard returned 3 and clobbered the file anyway — a refusal "
            "that still writes is worse than no refusal, because it reads as safe")

        # and the escape hatch really escapes: a guard with no --force is a
        # wall, and a wall gets worked around rather than satisfied.
        rc = cmd_read("HEAD", "ledger.md", out=str(_dirty), force=True, repo=td)
        assert rc == 0, "--force did not override the refusal"
        assert "PEER EDIT NOT YET COMMITTED" not in _dirty.read_text(
            encoding="utf-8"), "--force returned 0 and did not overwrite"
        _dirty.write_text(_keep, encoding="utf-8")
